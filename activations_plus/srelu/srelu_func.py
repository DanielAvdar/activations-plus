...
class SReLU(torch.nn.Module):
    ...
    def __init__(self, lower_threshold: float = -1.0, upper_threshold: float = 1.0) -> None:
        """Represents a Smooth Rectified Linear Unit (SReLU) activation function
        with user-defined thresholds. This activation function applies constraints
        on the input values, where inputs below a specified lower threshold or
        above an upper threshold are clipped. The SReLU is a piecewise linear
        activation function primarily used in neural networks.

        :param lower_threshold: The minimum value an input can take after being
            passed through the activation function.
        :param upper_threshold: The maximum value an input can take after being
            passed through the activation function.
    ...
    def forward(self, x: Tensor) -> Tensor:
        """Applies a thresholding operation to the input tensor, clipping values that are
        below or above the specified thresholds.

        If a value in the input tensor is less than the `lower_threshold`, it is
        replaced with `lower_threshold`. If a value exceeds the `upper_threshold`,
        it is replaced with `upper_threshold`. Values in between these thresholds
        remain unchanged.
