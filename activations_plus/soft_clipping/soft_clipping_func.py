...
class SoftClipping(torch.nn.Module):
    ...
    def __init__(self, x_min=-1.0, x_max=1.0, clip_func=torch.sigmoid):
        ...
        The class allows the customization of the clipping function as well as the range
        for clipping, which makes it applicable in a variety of machine learning tasks
        requiring such transformations.
        ...
        :type x_max: float
        :param clip_func: A differentiable function used to softly clip the input values.
                          This function determines the softness and shape of the transition
                          between unclipped and clipped regions. Defaults to torch.sigmoid.
